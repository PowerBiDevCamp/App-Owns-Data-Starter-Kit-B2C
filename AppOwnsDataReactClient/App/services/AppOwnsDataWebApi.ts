...
export default class AppOwnsDataWebApi {
  ...
  private static GetAccessToken = async (): Promise<string> => {
    ...
    if (account) {
      ...
      try {
        ...
        authResult = await msalInstance.acquireTokenSilent({ scopes: AppOwnsDataApiPermissionScopes, account: account });
        console.log("GetAccessToken  - Got access token from local cache");
      }
      catch {
        // if access token not available in cache, interact with user to acquire new access token 
        authResult = await msalInstance.acquireTokenPopup({ scopes: AppOwnsDataApiPermissionScopes, account: account });
        console.log("GetAccessToken  - Got access token from acrsso network");
      }
  ...
  static LoginUser = async (): Promise<void> => {
  ...
  static DeleteMyAccount = async (): Promise<void> => {

    var accessToken: string = await AppOwnsDataWebApi.GetAccessToken();

    var restUrl = AppOwnsDataWebApi.ApiRoot + "DeleteMyAccount/";
